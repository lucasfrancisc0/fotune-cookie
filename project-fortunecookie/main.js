...
const cookieOpen = document.querySelector(".cookieopened")
const cookieStart = document.querySelector(".cookiestart")
...
//Eventos
cookieStart.addEventListener('click', hiddenScreen);
btnReset.addEventListener('click', returnPage);

//Funções

function randomPhrases(event) {
  let indice = Math.round(Math.random() * luckyPhrases.length);
  cookieOpen.querySelector("h3").innerText = `${luckyPhrases[indice]}`;
}

function hiddenScreen(event) {
  cookieClosed.classList.add("hide");
  cookieOpen.classList.remove("hide");
  return randomPhrases();
}

function returnPage(event) {
  cookieOpen.classList.add("hide");
